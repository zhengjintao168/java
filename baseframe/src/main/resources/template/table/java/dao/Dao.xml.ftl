...
<mapper namespace="${packageName}.dao.${classPrefix}Dao" >
    ...
    <select id="query${classPrefix}List" parameterType="${classPrefix}Model" resultType="${classPrefix}Model" >
        select *
    ...
    <insert id="add${classPrefix}"  parameterType="${classPrefix}Model" >
        ...
        insert into ${tableName}(<#list columnList as column><#if column._isAdd || column.fieldName == "id" || column.fieldName == "createTime"><#if isInsertNeedComma> , </#if>${column.columnName}<#assign isInsertNeedComma=true></#if></#list>)
        <#assign isValueNeedComma=false>
        values(<#list columnList as column><#if column.fieldName == "id" >uuid() , <#elseif column.fieldName == "createTime" > , now()<#elseif column._isAdd ><#if isValueNeedComma> , </#if><#assign isValueNeedComma=true><@mapperEl column.fieldName /></#if></#list>);
    </insert>
